#!/bin/bash
# ==========================================
# AI创意协作平台 - aijiayuan.top MySQL健康检查脚本
# ==========================================
...
check_mysql_database() {
    ...
    if command -v mysql &> /dev/null; then
        ...
        # 使用Docker中的MySQL客户端
        if docker run --rm mysql:8.0 mysql -h"$MYSQL_HOST" -P"$MYSQL_PORT" -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" \
           -e "SELECT 1;" "$MYSQL_DATABASE" &>/dev/null; then
            log "✅ MySQL数据库连接正常 (通过Docker)"
            return 0
        else
            log_error "❌ MySQL数据库连接失败"
            return 1
...
check_docker_services() {
    ...
    # 检查主要服务（不包括PostgreSQL）
    for service in app redis nginx; do
        if docker-compose -f /opt/ai-marketplace/docker-compose.mysql.yml ps "$service" 2>/dev/null | grep -q "Up"; then
...
main() {
    log "=========================================="
    log "🔍 开始 aijiayuan.top 健康检查 (MySQL版本)"
    log "=========================================="
    ...
    check_external_services && ((check_passed++))
    check_mysql_database && ((check_passed++))
...
# 显示使用帮助
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    echo "aijiayuan.top 健康检查脚本 (MySQL版本)"
    echo ""
    echo "用法: $0 [选项]"
    echo ""
    echo "选项:"
