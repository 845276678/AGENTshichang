...
exports.TemplateManager = exports.PERSONALITY_INTERACTIONS = exports.DIALOGUE_TEMPLATES = void 0;
// 对话模板数据库
...
// 个性化互动模板
exports.PERSONALITY_INTERACTIONS = {
    'tech-pioneer-alex': {
...
class TemplateManager {
    ...
    initializeTemplates() {
        // 按触发事件分组模板
        exports.DIALOGUE_TEMPLATES.forEach(template => {
    ...
    // 生成基于模板的对话内容
    generateFromTemplate(templateType, context) {
        ...
        // 选择合适的模板
        const suitableTemplates = templates.filter(template => this.isTemplateApplicable(template, context));
    ...
    // 选择合适的变体
    selectVariant(template, context) {
        const applicableVariants = template.variants.filter(variant => this.isVariantApplicable(variant, context));
    ...
    // 检查变体是否适用
    isVariantApplicable(variant, context) {
    ...
    // 个性化内容
    personalizeContent(content, context) {
        // 替换占位符
    ...
    // 生成竞价金额
    generateBidAmount(range, context) {
        const [min, max] = range;
    ...
    // 获取角色ID
    getPersonaId(personaType, context) {
        if (personaType === 'any') {
    ...
    // 生成个性化互动
    generatePersonalityInteraction(personaId, interactionType, context) {
        const personality = exports.PERSONALITY_INTERACTIONS[personaId];
        if (!personality)
            return '';
