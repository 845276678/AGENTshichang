...
export interface DialogueStrategy {
  type: 'real_ai' | 'scripted' | 'hybrid';
  trigger: string;
  priority: 'high' | 'medium' | 'low';
  cost_impact: number;
}

export interface AICallTrigger {
  phase: 'warmup' | 'discussion' | 'bidding' | 'prediction' | 'result';
  event: string;
  requires_real_ai: boolean;
  fallback_script?: string;
}

// 混合对话策略配置
export const HYBRID_DIALOGUE_STRATEGY = {
  // 真实AI调用的关键节点
  real_ai_calls: [
    {
...
export class DialogueDecisionEngine {
  ...
  private async generateRealAIDialogue(
    context: DialogueContext,
    strategy: DialogueStrategy
  ): Promise<DialogueResponse> {
    try {
      const aiCall = HYBRID_DIALOGUE_STRATEGY.real_ai_calls.find(
        call => call.trigger === context.trigger
      );
  ...
  private generateScriptedDialogue(
    context: DialogueContext,
    strategy: DialogueStrategy
  ): DialogueResponse {
    const scriptConfig = HYBRID_DIALOGUE_STRATEGY.scripted_content.find(
      script => script.trigger === context.trigger
    );
    ...
    const dialogueContent = this.templateManager.generateFromTemplate(
      scriptConfig.templates,
      context
    );
  ...
  private async generateHybridDialogue(
    context: DialogueContext,
    strategy: DialogueStrategy
  ): Promise<DialogueResponse> {
...
export class CostTracker {
  ...
  recordAPICall(userId: string, cost: number): void {
    ...
    // 设置用户冷却时间
    this.userCooldowns.set(userId, Date.now() + HYBRID_DIALOGUE_STRATEGY.cost_control.user_cooldown * 1000);
  }

  isOverBudget(): boolean {
    const today = this.getTodayKey();
    const usage = this.dailyUsage.get(today) || 0;
    return usage >= HYBRID_DIALOGUE_STRATEGY.cost_control.daily_budget * HYBRID_DIALOGUE_STRATEGY.cost_control.fallback_threshold;
  }
...
export default DialogueDecisionEngine;